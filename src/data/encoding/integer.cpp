...
namespace data::encoding {
    ...
    namespace decimal {
        ...
        std::strong_ordering string::operator<=>(const string &n) const {
            if (!this->valid() || !n.valid()) throw std::invalid_argument{"invalid decimal string"};
        ...
        string string::operator+(const string& n) const {
            ...
            return decimal::write(N::read(*this) + N::read(n));
        }
        ...
        string string::operator<<(int i) const {
            if (!this->valid()) throw std::invalid_argument{"invalid dec string"};
            return decimal::write(N::read(*this) << i);
        }
        ...
        string string::operator&(const string& n) const {
            if (!this->valid()) throw std::invalid_argument{"invalid dec string"};
            if (!n.valid()) throw std::invalid_argument{"invalid dec string"};
            return decimal::write(math::N_bytes<endian::little>::read(*this) & math::N_bytes<endian::little>::read(n));
        }
        ...
        string string::operator%(const string &x) const {
            return decimal::write(decimal::divide(*this, N::read(x)).Remainder);
        }
    ...
    namespace signed_decimal {
        ...
        std::strong_ordering string::operator<=>(const string& x) const {
            if (!this->valid() || !x.valid()) throw std::invalid_argument{"invalid hexidecimal string"};
            math::sign na = sign(*this);
            math::sign nb = sign(x);
            
            return na != nb ? na <=> nb :
                na == math::negative && nb == math::negative ? 
                decimal::N_compare(string_view{x.data() + 1, x.size() - 1}, string_view{this->data() + 1, this->size() - 1}) : 
                decimal::N_compare(string_view{this->data(), this->size()}, string_view{x.data(), x.size()});
        ...
        string string::operator+(const string& n) const {
            if (!this->valid()) throw std::invalid_argument{"invalid +/-dec string"};
            if (!n.valid()) throw std::invalid_argument{"invalid +/-dec string"};
            return signed_decimal::write(Z::read(*this) + Z::read(n));
        }
        ...
        string string::operator<<(int i) const {
            if (!this->valid()) throw std::invalid_argument{"invalid +/-dec string"};
            return signed_decimal::write(Z::read(*this) << i);
        }
        ...
        string string::operator-() const {
            string x;
            if (negative(*this)) {
                x.resize(this->size() - 1);
                std::copy(this->begin() + 1, this->end(), x.begin());
            } else {
                x[0] = '-';
                x.resize(this->size() + 1);
                std::copy(this->begin(), this->end(), x.begin() + 1);
            }
            return x;
        }
