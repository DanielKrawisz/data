...
namespace data::encoding::base58 {
    ...
    std::strong_ordering operator<=>(const string &m, const string &n) {
        if (!m.valid() || !n.valid()) throw std::invalid_argument{"invalid base 58 string"};
    ...
    string &operator++(string &x) {
        if (!x.valid()) throw std::invalid_argument{"invalid hexidecimal string"};
    ...
    string operator<<(const string& m, int i) {
        if (!m.valid()) throw std::invalid_argument{"invalid hexidecimal string"};
