...
namespace data::math {
    ...
    template <typename A, bit_integral B>
    A inline binary_accumulate_times (const A &x, const B &y) {
        if (y < 0) return -binary_accumulate_times (x, -y);
        return binary_accumulate<A, B> {&data::plus<A, A>, &data::mul_2<A>} (A {0}, x, y);
    }

    template <typename A, bit_integral B>
    A inline binary_accumulate_pow (const A &x, const B &y) {
        if (y < 0) throw exception {} << "cannot take a negative power";
        return binary_accumulate<A, B> {&data::times<A, A>, &data::square<A>} (A {1}, x, y);
    }
    ...
    A inline binary_accumulate_times_mod (const A &x, const B &y, const Mod &z) {
        if (z < 0) throw exception {} << "cannot mod by a negative number";
        if (y < 0) return -binary_accumulate_times_mod (x, -y, z);
        return binary_accumulate<A, B> {[z] (const A &a, const A &b) -> A {
    ...
    template <typename A, bit_integral B, group_integral Mod>
    A inline binary_accumulate_pow_mod (const A &x, const B &y, const Mod &z) {
        if (z < 0) throw exception {} << "cannot mod by a negative number";
        if (y < 0) throw exception {} << "cannot take a negative power";
        return binary_accumulate<A, B> {[z] (const A &a, const A &b) {
            return data::times_mod<A, A, Mod> (a, b, z);
        }, [z] (const A &a) {
            return data::square_mod<A, Mod> (a, z);
        }} (A {1}, x, y);
    }
    ...
    struct pow<A, Exp> {
        A operator () (const A &x, const Exp &y) {
            return binary_accumulate_pow (x, y);
        }
    };

    template <typename A, bit_integral B, group_integral Mod>
    struct times_mod<A, B, Mod> {
        A operator () (const A &x, const B &y, const Mod &z) {
            return binary_accumulate_times_mod (x, y, z);
        }
    };

    template <typename A, bit_integral Exp, group_integral Mod> struct pow_mod<A, Exp, Mod> {
        A operator () (const A &x, const Exp &y, const Mod &z) {
