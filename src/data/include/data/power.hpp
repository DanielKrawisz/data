...
namespace data::math {
    ...
    A inline binary_accumulate_times_mod (const A &x, const B &y, const nonzero<Mod> &z) {
    ...
    template <typename A, bit_integral B, group_integral Mod>
    auto inline binary_accumulate_pow_mod (const A &x, const B &y, const nonzero<Mod> &z) -> decltype (data::abs (z.Value)) {
        if (z.Value < 0) throw exception {} << "cannot mod by a negative number";
        if (y < 0) throw exception {} << "cannot take a negative power";
    ...
    template <typename A, bit_integral Exp, group_integral Mod> struct pow_mod<A, Exp, Mod> {
        auto operator () (const A &x, const Exp &y, const nonzero<Mod> &z) -> decltype (data::abs (z.Value)) {
