...
namespace data {
    ...
    struct binary_search_map : tree {
        ...
        retrieved<value> operator [] (const key &);
        inserted<value> operator [] (const key &) const;

        const unref<value> *contains (const key &) const;
        unref<value> *contains (const key &);
        bool contains (const entry &e) const;

        // if a key already exists, the default behavior is NOT to replace that key.
        binary_search_map insert (const key &, const value &) const;
        ...
        binary_search_map insert (const key &k, const value &v,
            function<value (const value &old_v, const value &new_v)> already_exists) const;
    ...
    private:
        static value default_key_already_exists (const value &old_v, const value &new_v);
    };
    ...
    template <ordered key, typename value, functional::search_tree<data::entry<const key, value>> tree>
    requires interface::has_insert_method<tree, data::entry<const key, value>>
    value inline binary_search_map<key, value, tree>::default_key_already_exists (const value &, const value &) {
