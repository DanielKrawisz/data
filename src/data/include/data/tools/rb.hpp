...
namespace data::RB {
    ...
    template <sortable V, functional::buildable_tree<colored<V>> T>
    tree<V, T> inline tree<V, T>::insert (inserted<V> v) const {
        return RB::insert (*this, v);
    }
    ...
    template <sortable V, functional::buildable_tree<colored<V>> T>
    unref<V> inline *tree<V, T>::contains (inserted<V> x) const {
        auto v = binary_search_tree<colored<V>, T>::contains (colored<V> {color::black, v});
