...
namespace {
    ...
    using unsigned_bounded_8_little = data::math::number::bounded::number<std::array<data::byte, 4>, 4, data::endian::little, false>;
    using unsigned_bounded_8_big = data::math::number::bounded::number<std::array<data::byte, 4>, 4, data::endian::big, false>;
    using signed_bounded_8_little = data::math::number::bounded::number<std::array<data::byte, 4>, 4, data::endian::little, true>;
    using signed_bounded_8_big = data::math::number::bounded::number<std::array<data::byte, 4>, 4, data::endian::big, true>;

    TEST(BoundedTest, BoundedReadString) {
        
        EXPECT_NO_THROW(signed_bounded_8_big{   "-1"});
        EXPECT_NO_THROW(signed_bounded_8_little{"-1"});
        
        EXPECT_THROW(unsigned_bounded_8_big{   "-1"}, std::invalid_argument);
        EXPECT_THROW(unsigned_bounded_8_little{"-1"}, std::invalid_argument);
        
        EXPECT_NO_THROW(signed_bounded_8_big{   "-0x01"});
        EXPECT_NO_THROW(signed_bounded_8_little{"-0x01"});
        ...
        EXPECT_NO_THROW(signed_bounded_8_big{   "0xffffffffffffffff"});
        EXPECT_NO_THROW(signed_bounded_8_little{"0xffffffffffffffff"});
