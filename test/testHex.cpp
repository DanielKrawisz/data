...
namespace data::encoding::hex {
    ...
    TEST(HexTest, HexValidString) {
        ASSERT_TRUE(encoding::hex::valid("012345abcd"));
        ASSERT_TRUE(encoding::hex::valid("012345ABCD"));
        ASSERT_FALSE(encoding::hex::valid("012345aBcD"));
        ASSERT_FALSE(encoding::hex::valid("012345abc"));
        ASSERT_FALSE(encoding::hex::valid("012345aHcd"));
    }
    ...
    TEST(HexTest, HexWriteBytes) {
        ...
        string written = write(data::bytes_view{testVector.data(), testVector.size()}, upper);
        ASSERT_STREQ(written.c_str(),"0063EA172D63808C");
    }

    TEST(HexTest, HexWritePubKey) {
        std::array<byte, 33> a({0x80,0x0C,0x28,0xFC,0xA3,0x86,0xC7,0xA2,0x27,0x60,0x0B,
            0x2F,0xE5,0x0B,0x7C,0xAE,0x11,0xEC,0x86,0xD3,0xBF,0x1F,0xBE,0x47,0x1B,0xE8,0x98,0x27,0xE1,0x9D,0x72,0xAA,0x1D});
        string testString = write(data::bytes_view{a.data(), 33}, upper);
