...
namespace data {
    ...
    void test_decrement_signed(const string &given, const string &expected) {
        ...
        EXPECT_EQ(math::number::decrement(g), e);
        EXPECT_EQ(math::number::decrement(Z::read(g)), Z::read(e));
        EXPECT_EQ(math::number::decrement(Z_bytes_little::read(g)), Z_bytes_little::read(e));
        EXPECT_EQ(math::number::decrement(Z_bytes_big::read(g)), Z_bytes_big::read(e));
    }
    ...
    void test_decrement_unsigned(const string &given, const string &expected) {
        ...
        EXPECT_EQ(math::number::decrement(g), e);
        EXPECT_EQ(math::number::decrement(N::read(g)), N::read(e));
        
        EXPECT_EQ(math::number::decrement(N_bytes_little::read(g)), N_bytes_little::read(e));
        EXPECT_EQ(math::number::decrement(N_bytes_big::read(g)), N_bytes_big::read(e));
        
        auto gg = base58_uint::read(g);
        auto ee = base58_uint::read(e);
        auto ggd = math::number::decrement(gg);
        EXPECT_EQ(ggd, ee);
        
        EXPECT_EQ(math::number::decrement(hex_uint::read(g)), hex_uint::read(e));
        
    }
    
    void test_increment_signed_final(const dec_int &g, const dec_int &e) {
        
        EXPECT_EQ(math::number::increment(g), e);
        EXPECT_EQ(math::number::increment(Z::read(g)), Z::read(e));
        EXPECT_EQ(math::number::increment(Z_bytes_little::read(g)), Z_bytes_little::read(e));
        EXPECT_EQ(math::number::increment(Z_bytes_big::read(g)), Z_bytes_big::read(e));
    ...
    void test_increment_unsigned(const string &given, const string &expected) {
        ...
        EXPECT_EQ(math::number::increment(g), e);
        EXPECT_EQ(math::number::increment(N::read(g)), N::read(e));
        EXPECT_EQ(math::number::increment(N_bytes_little::read(g)), N_bytes_little::read(e));
        EXPECT_EQ(math::number::increment(N_bytes_big::read(g)), N_bytes_big::read(e));
        
        auto b58g = base58_uint::read(g);
        auto b58e = base58_uint::read(e);
        auto b58i = math::number::increment(b58g);
        EXPECT_EQ(b58i, b58e);
        EXPECT_EQ(math::number::increment(hex_uint::read(g)), hex_uint::read(e));
    ...
    void test_divide_unsigned(const string &left, const string &right, const string &expected) {
        
    }
    
    void test_divide_signed(const string &left, const string &right, const string &expected) {
        
    }
    
    TEST(DecimalTest, TestDecimalDivide) {
