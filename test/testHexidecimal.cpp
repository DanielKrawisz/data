...
namespace data::math::number {
    ...
    template <complement n> struct test_bit_shift<n> {
        test_bit_shift(string given, byte bits, string expected_left, string expected_right) {
            ...
            auto g = hex<n>{given};
            std::cout << " " << g << " << " << std::dec << uint64(bits) << " -> " << l << std::endl;
            auto x = g << bits;
            std::cout << " " << g << " << " << std::dec << uint64(bits) << " -> " << x << std::endl;
            EXPECT_EQ(x, l);
            EXPECT_EQ(x >> bits, g);
            EXPECT_EQ(g >> bits, r);
    ...
    template <complement n> struct test_plus<n> {
        test_plus(string left, string right, string expected) {
            auto l = hex<ones>::read(left);
            auto r = hex<ones>::read(right);
            auto e = hex<ones>::read(expected);
            EXPECT_EQ(l + r, e);
        }
    };
    ...
    TEST(HexidecimalTest, TestHexidecimalTimes) {
        test_times<nones, ones, twos>("0x", "0x", "0x");
    }
