...
namespace data::encoding::decimal {
    ...
    struct string : std::string {
        ...
        explicit string(string_view x) : std::string{x} {}
        explicit string(std::string &&x) : std::string{decimal::valid(x) ? x : ""} {}
        string(uint64);
...
namespace data::encoding::signed_decimal {
    ...
    struct string : std::string {
        string() : std::string{"0"} {};
        explicit string(string_view x) : std::string{signed_decimal::valid(x) ? x : ""} {}
...
namespace data::math {
    ...
    dec_uint inline abs<dec_int>::operator()(const dec_int &x) {
        if (encoding::signed_decimal::negative(x)) return dec_uint{string_view{x.data() + 1, x.size() - 1}};
        return dec_uint{x};
    }
