...
namespace data::net::HTTP {
    struct client_blocking {
        ...
        response GET (list<UTF8> path, list<entry<UTF8, UTF8>> params = {});
        response POST (list<UTF8> path, map<header, ASCII> headers, string body);
        response POST_form_data (list<UTF8> path, map<header, ASCII> form_data = {});
    ...
    struct client_async : std::enable_shared_from_this<client_async> {
        ...
        void GET (handler<const response &>, list<UTF8> path, list<entry<UTF8, UTF8>> params = {});
        void POST (handler<const response &>, list<UTF8> path, map<header, ASCII> headers, string body);
        void POST_form_data (handler<const response &>, list<UTF8> path, map<header, ASCII> form_data = {});
    ...
    response inline client_blocking::GET (list<UTF8> path, list<entry<UTF8, UTF8>> params) {
    ...
    response inline client_blocking::POST (list<UTF8> path, map<header, ASCII> headers, string body) {
        return (*this) (REST.POST (path, headers, body));
    }

    void inline client_async::POST (
        handler<const response &> on_receive,
        list<UTF8> path, map<header, ASCII> headers,
        string body) {
        return (*this) (on_receive, REST.POST (path, headers, body));
    }
    
    // POST form data
    response inline client_blocking::POST_form_data (list<UTF8> path, map<header, ASCII> form_data) {
