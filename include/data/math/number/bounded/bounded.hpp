...
template <bool u, endian::order, size_t size>
requires(size >= 8) struct bounded;
...
math::number::bounded<u, r, x>
increment(const math::number::bounded<u, r, x> &);
template <bool u, endian::order r, size_t x>
math::number::bounded<u, r, x>
decrement(const math::number::bounded<u, r, x> &);
...
template <bool x, endian::order r, size_t n, bool y, endian::order o, size_t z>
std::weak_ordering operator<=>(const bounded<x, r, n> &,
                               const bounded<y, o, z> &);
...
template <bool u, endian::order r, size_t x>
bool is_zero(const bounded<u, r, x> &);
...
struct bounded<false, r, size> : oriented<r, byte, size> {
  ...
  bounded() : oriented<r, byte, size>() { this->fill(0x00); }
  ...
  static bounded read(string_view s) { return bounded{s}; }
  ...
  bounded operator/(const bounded &n) const { return divide(n).Quotient; }

  bounded operator%(const bounded &n) const { return divide(n).Remainder; }
  ...
  operator bytes_view() const { return bytes_view(*this); }

  explicit operator N() const { return N(N_bytes<r>(*this)); }

  explicit operator double() const { return double(N(*this)); }
    ...
    if (n.size() <= size) {
      if (r == endian::little) {
        ...
        std::copy(n.begin(), n.end(),
                  byte_array<size>::begin() + (size - n.size()));
      }
    }
    if (n > N_bytes<r>{max()})
      throw std::out_of_range{"N_bytes too big"};
    if (r == endian::little) {
      std::copy(n.begin(), n.begin() + size, byte_array<size>::begin());
    } else {
      std::copy(n.begin() + (n.size() - size), n.end(),
                byte_array<size>::begin());
    }
  }
...
private:
  bounded(const sint<r, size> &) {
    throw method::unimplemented{
        "bounded<size, o, false>{bounded<size, o, true>}"};
  }
...
struct bounded<true, r, size> : oriented<r, byte, size> {
  ...
  bounded() : oriented<r, byte, size>{} { this->fill(0); }
  ...
  static N_bytes<r> modulus() { return uint<r, size>::modulus(); }
  ...
  explicit operator Z() const { return Z(Z_bytes<r>(*this)); }

  explicit operator double() const { return double(Z(*this)); }

private:
  explicit bounded(const Z_bytes<r> &z) {
    if (z > Z_bytes<r>{max()} || z < Z_bytes<r>{min()})
      throw std::out_of_range{"Z_bytes too big"};
...
template <bool x, endian::order r, size_t n, bool y, endian::order o, size_t z>
bool operator==(const bounded<x, r, n> &a,
                const endian::arithmetic<y, o, z> &b) {
  return (a <=> b) == 0;
}
...
template <bool x, endian::order r, size_t n, bool y, endian::order o, size_t z>
std::weak_ordering operator<=>(const bounded<x, r, n> &a,
                               const endian::arithmetic<y, o, z> &b) {
  return a <=> bounded<y, o, z>(b);
}

template <endian::order r, size_t x>
bool inline is_positive(const uint<r, x> &n) {
  return !is_zero(n);
}

template <endian::order r, size_t x>
bool inline is_positive(const sint<r, x> &n) {
  return !is_negative(n) && !is_zero(n);
}

template <endian::order r, size_t x>
bool inline is_negative(const uint<r, x> &n) {
  return false;
}
...
template <endian::order r, size_t x>
math::sign inline sign(const math::uint<r, x> &z) {
  return math::arithmetic::N_sign(z.words());
}

template <endian::order r, size_t x>
math::sign inline sign(const math::sint<r, x> &z) {
  return math::arithmetic::Z_sign_ones(z.words());
}

template <bool u, endian::order r, size_t x>
math::number::bounded<u, r, x> inline increment(
    const math::number::bounded<u, r, x> &n) {
  auto z = n;
  return ++z;
}

template <bool u, endian::order r, size_t x>
math::number::bounded<u, r, x> inline decrement(
    const math::number::bounded<u, r, x> &n) {
  auto z = n;
  return --z;
}
...
template <size_t size, endian::order o> struct abs<uint<o, size>> {
  uint<o, size> operator()(const uint<o, size> &i) { return i; }
};

template <size_t size, endian::order o> struct abs<sint<o, size>> {
...
template <endian::order r, size_t size>
std::weak_ordering inline operator<=>(const uint<r, size> &a,
                                      const uint<r, size> &b) {
  return arithmetic::N_compare(a.words(), b.words());
}

template <bool x, endian::order r, size_t n, bool y, endian::order o, size_t z>
std::weak_ordering inline operator<=>(const bounded<x, r, n> &,
                                      const bounded<y, o, z> &) {
  throw 0;
}
...
template <endian::order r, size_t x>
sint<r, x> inline operator+(const sint<r, x> &a, int64 b) {
  return a + sint<r, x>(b);
}
...
template <endian::order r, size_t x>
uint<r, x> inline &operator+=(uint<r, x> &a, uint64 b) {
  return a += uint<r, x>(b);
}
...
bounded<u, r, size> inline &operator++(bounded<u, r, size> &x) {
  data::arithmetic::plus<byte>(x.words().end(), x.words().begin(), 1,
                               x.words().begin());
  return x;
}
...
template <bool u, endian::order r, size_t size>
bounded<u, r, size> inline &operator--(bounded<u, r, size> &x) {
  data::arithmetic::minus<byte>(x.words().end(), x.words().begin(), 1,
                                x.words().begin());
  return x;
}

template <bool is_signed, endian::order r, size_t size>
bounded<is_signed, r, size> inline
operator<<(const bounded<is_signed, r, size> &z, int i) {
  auto n = z;
  return n <<= i;
}
...
void shift_right(byte_array<size> &n, uint32 i, byte fill) {
  if (r == endian::big)
    data::arithmetic::bit_shift_right(n.rbegin(), n.rend(), i, fill);
  else
    data::arithmetic::bit_shift_right(n.begin(), n.end(), i, fill);
}

template <endian::order r, size_t size>
void shift_left(byte_array<size> &n, uint32 i, byte fill) {
  if (r == endian::big)
    data::arithmetic::bit_shift_left(n.begin(), n.end(), i, fill);
  else
    data::arithmetic::bit_shift_left(n.rbegin(), n.rend(), i, fill);
}
} // namespace

template <bool is_signed, endian::order r, size_t size>
bounded<is_signed, r, size> inline &operator<<=(bounded<is_signed, r, size> &n,
                                                int i) {
...
template <endian::order o, size_t size> uint<o, size> uint<o, size>::min() {
  bounded b{};
...
template <endian::order o, size_t size> uint<o, size> uint<o, size>::max() {
  bounded b{};
  for (int i = 0; i <= size; i++)
    b[i] = 0xff;
  return b;
...
template <endian::order o, size_t size> sint<o, size> sint<o, size>::min() {
  bounded b{};
  b.words()[-1] = 0x80;
  return b;
}

template <endian::order o, size_t size> sint<o, size> sint<o, size>::max() {
  bounded b{-1};
  b.words()[-1] = 0x7f;
  return b;
}
...
template <bool u, endian::order r, size_t size>
bounded<u, r, size> inline operator&(const bounded<u, r, size> &a,
                                     const bounded<u, r, size> &b) {
  bounded<u, r, size> x;
  data::arithmetic::bit_and<byte>(x.words().end(), x.words().begin(),
                                  a.words().begin(), b.words().begin());
  return x;
}
...
bounded<u, r, size> inline operator|(const bounded<u, r, size> &a,
                                     const bounded<u, r, size> &b) {
  bounded<u, r, size> x;
  data::arithmetic::bit_or<byte>(x.words().end(), x.words().begin(),
                                 a.words().begin(), b.words().begin());
  return x;
}
...
bounded<u, r, size> inline operator^(const bounded<u, r, size> &a,
                                     const bounded<u, r, size> &b) {
  bounded<u, r, size> x;
  data::arithmetic::bit_xor<byte>(x.words().end(), x.words().begin(),
                                  a.words().begin(), b.words().begin());
  return x;
}

template <bool u, endian::order r, size_t size>
bounded<u, r, size> inline &operator&=(bounded<u, r, size> &a,
                                       const bounded<u, r, size> &b) {
  data::arithmetic::bit_and<byte>(
      a.words().end(), a.words().begin(),
      const_cast<const bounded<u, r, size> &>(a).words().begin(),
      b.words().begin());
  return a;
}
...
bounded<u, o, size> inline operator+(const bounded<u, o, size> &a,
                                     const bounded<u, o, size> &n) {
  bounded<u, o, size> x;
  data::arithmetic::plus<byte>(x.words().end(), x.words().begin(),
                               a.words().begin(), n.words().begin());
  return x;
}

template <bool u, endian::order o, size_t size>
bounded<u, o, size> inline &operator+=(bounded<u, o, size> &a,
                                       const bounded<u, o, size> &n) {
  data::arithmetic::plus<byte>(
      a.words().end(), a.words().begin(),
      const_cast<const bounded<u, o, size> &>(a).words().begin(),
      n.words().begin());
  return a;
}

template <bool u, endian::order o, size_t size>
bounded<u, o, size> operator-(const bounded<u, o, size> &a,
                              const bounded<u, o, size> &n) {
  bounded<u, o, size> z;
  ...
  ++z;
  bounded<u, o, size> x;
  data::arithmetic::minus<byte>(x.words().end(), x.words().begin(),
                                a.words().begin(), z.words().begin());
  return x;
}

template <bool u, endian::order o, size_t size>
bounded<u, o, size> &operator-=(bounded<u, o, size> &a,
                                const bounded<u, o, size> &n) {
  bounded<u, o, size> z;
  std::copy(n.begin(), n.end(), z.begin());
  z.bit_negate();
...
bounded<u, o, size> inline operator*(const bounded<u, o, size> &a,
                                     const bounded<u, o, size> &n) {
  bounded<u, o, size> x;
  data::arithmetic::times<byte>(x.words().end(), x.words().begin(),
                                a.words().begin(), n.words().begin());
  return x;
}
...
template <endian::order r, size_t size>
bounded<false, r, size> operator+(const bounded<false, r, size> &a,
                                  const bounded<false, r, size> &b) {
  bounded<false, r, size> n{};
  data::arithmetic::plus<byte>(n.words().end(), n.words().begin(),
                               a.words().begin(), b.words().begin());
  return n;
}

template <endian::order r, size_t size>
bounded<true, r, size> operator+(const bounded<true, r, size> &a,
                                 const bounded<true, r, size> &b) {
...
template <bool is_signed, endian::order r, size_t size>
bool operator==(const bounded<is_signed, r, size> &a,
                const bounded<is_signed, r, size> &b) {
...
template <bool u, endian::order r, size_t x>
bool inline is_zero(const bounded<u, r, x> &z) {
  return arithmetic::ones_is_zero(z.words());
}

template <endian::order r, size_t x>
bool inline is_negative(const sint<r, x> &z) {
